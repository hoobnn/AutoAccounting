...
/**
 * TensorFlow Lite 分类抽象；子类装载具体 .tflite 并实现 [predict]。
 * （原 columbus GestureSensor TfClassifier）
 */
open class TfClassifier {

    /** 默认空实现；由 [TapTfClassifier] 覆盖。 */
    open fun predict(input: ArrayList<Float>, size: Int): ArrayList<ArrayList<Float>> =
        ArrayList()

    /** 旧版扁平输入拓扑（model 11）。 */
    protected fun predict11(
        interpreter: Interpreter,
        input: ArrayList<Float>,
        size: Int,
